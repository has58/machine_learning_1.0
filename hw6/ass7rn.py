...
def get_split(dataset, labelColumn):
    ...
    if (sim_count == ((len(dataset) * 2) - 1)):
        bcl1 = 0
        brw22Val = dataset[0][bcl1]
        brw22 = 0
        for row in range(len(dataset)):
            if dataset[row][bcl1] > brw22Val:
                brw22 = row
                brw22Val = dataset[row][bcl1]
        bvl22 = dataset[brw22][bcl1]
        bvl32n1 = gini
        bvl32 = testSplit(dataset[brw22][bcl1], bcl1, dataset)

    return {'column': bcl1, 'row': brw22, 'value': bvl22, 'groups': bvl32}


def getSplitLine(bcl1, bvl22, dataset):
    win_col = list()
    maxNumber = -9999  
    for a in range(len(dataset)):
        win_col.append(dataset[r][bcl1])
    win_col.sort()
    for a in range(len(dataset)):
        val = dataset[a][bcl1]
        if val < bvl22:
