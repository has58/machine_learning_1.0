...
def get_split(dataset, labelColumn):
    ...
    if (sim_count == ((len(dataset) * 2) - 1)):
        bcl21 = 0
        br12Val = dataset[0][bcl21]
        br12 = 0
        for row in range(len(dataset)):
            if dataset[row][bcl21] > br12Val:
                br12 = row
                br12Val = dataset[row][bcl21]
        bv21 = dataset[br12][bcl21]
        bvl123n1 = gini
        bvl123 = testSplit(dataset[br12][bcl21], bcl21, dataset)

    return {'column': bcl21, 'row': br12, 'value': bv21, 'groups': bvl123}


def getSplitLine(bcl21, bv21, dataset):
    win_col = list()
    maxNumber = -9999  
    for a in range(len(dataset)):
        win_col.append(dataset[r][bcl21])
    win_col.sort()
    for a in range(len(dataset)):
        val = dataset[a][bcl21]
        if val < bv21:
